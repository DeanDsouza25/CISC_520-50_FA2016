...
def getWordFeatures(tWordList):
    ...
    # Sorting based on word scores and then creating set of best word features 
    bestVals = sorted(wScores.iteritems(), key=lambda (w,s): s, reverse=True)
    return bestVals

# Using the getWordsInTweets() and getWordFeatures() to get the best features to use in the FeatExt()
tweetsFeat = getWordFeatures(getWordsInTweets(trainTweets))

def getNFeats(bestVals,num):
    bestV = bestVals[:num]
    bestFeats = set([w for (w,s) in bestV])
    return bestFeats

def evaluateDTC(num):
    # Getting 'n' best feats
    tweetsFeats = getNFeats(tweetsFeat,num)
    ...
    for i, (feature,sentiment) in enumerate(testTweetsApplied):
        ...
        pred= classifierDT.classify(feature)
        testFeatSet[pred].add(i)
        
    print("Evaluating best %d features: " %(num))
    print(nltk.classify.accuracy(classifierDT, testTweetsApplied))
    ...
    print(classifierDT.pseudocode(depth=3))

print("For %d training data and %d testing data: " % (len(trainTweets),len(testTweets)))
